/* Generated by pecans-spec. Do not edit. */
import { Call, Module } from "../../runtime";
import {
    ListFollowersResponse,
    ListFollowsResponse,
    ProfileDetailsResponse,
    ProfileGetResponse,
    TCResponse,
} from "../types";

export class ProfileModule extends Module {
    /**
     * Check whether a profile exists and resolve its canonical id.
     *
     * @param id - Username to look up.
     * @returns The user's id, or `ok: false` with error `NOT_FOUND`.
     */
    public get(id: string): Call<ProfileGetResponse> {
        return new Call<ProfileGetResponse>(this.client, {
            fn: "profile.get",
            payload: {
                id: id,
            },
        });
    }

    /**
     * Fetch one field of a user's profile.
     *
     * @param user - Username whose profile to read.
     * @param field - Field name, e.g. `blurb`, `title`, `followers`, `joinTime`, `contact.discord`.
     * @returns The field name and its value.
     */
    public details(user: string, field: string): Call<ProfileDetailsResponse> {
        return new Call<ProfileDetailsResponse>(this.client, {
            fn: "profile.details",
            payload: {
                user: user,
                field: field,
            },
        });
    }

    /**
     * Set one field on your own profile.
     *
     * @param field - Field name, e.g. `blurb`, `contact.web`.
     * @param value - New value for the field.
     */
    public edit(field: string, value: string): Call<TCResponse> {
        return new Call<TCResponse>(this.client, {
            fn: "profile.edit",
            payload: {
                field: field,
                value: value,
            },
        });
    }

    /**
     * Follow or unfollow another user.
     *
     * @param id - Username to follow or unfollow.
     * @param follow - True to follow, false to unfollow.
     */
    public toggleFollow(id: string, follow: boolean): Call<TCResponse> {
        return new Call<TCResponse>(this.client, {
            fn: "profile.togglefollow",
            payload: {
                id: id,
                follow: follow,
            },
        });
    }

    /**
     * List the users who follow you.
     */
    public listFollowers(): Call<ListFollowersResponse> {
        return new Call<ListFollowersResponse>(this.client, {
            fn: "profile.listfollowers",
            payload: {},
        });
    }

    /**
     * List the users you follow (your subscriptions).
     */
    public listFollows(): Call<ListFollowsResponse> {
        return new Call<ListFollowsResponse>(this.client, {
            fn: "profile.listfollows",
            payload: {},
        });
    }
}
